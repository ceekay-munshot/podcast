...
function okLLM() {
  const args = JSON.stringify({
    synthesis: ['point'],
    takeaways: [{ title: 'T', detail: 'D' }],
    qa: [{ q: 'Q', a: 'A' }],
    moments: [{ title: 'M', timestamp: '—', whyItMatters: 'why' }],
...
describe('summarizeEpisode — shared store reuse', () => {
  it('serves a stored episode without any LLM call (L1-cold path)', async () => {
    ...
    const stored: SummarizeResult = { summary: { synthesis: ['cached'], takeaways: [], qa: [], moments: [] }, transcript: [] }
